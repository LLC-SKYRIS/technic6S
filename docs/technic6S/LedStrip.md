...
Адресуемая RGB-светодиодная лента типа **ws281x**, которая входит в наборы Technic, позволяет выставлять произвольные 24-битные цвета на каждый из отдельных светодиодов. Это позволяет сделать полет Technic более ярким. Для использования светодиодной ленты с OrangePi 5 Pro был установлен драйвер opi_ws2812_rt. Лента также позволяет визуально получать информацию о полетных режимах, этапе выполнения пользовательской программы и других событиях.

<img src="assets/0420.png" align="right" width="400px" alt="Technic Drone">

На образе [для OPi](ImageOPI.md) предустановлены необходимые модули для работы с лентой. Они позволяют:
...
> **Подсказка** Обратите внимание, что светодиодную ленту нужно питать от стабильного источника энергии. Если вы подключите питание напрямую к OPi, то это создаст слишком большую нагрузку на ваш микрокомпьютер. Для снятия нагрузки с OPi можно подключить питание к преобразователю BEC.

### Высокоуровневое управление лентой {#set_effect}

1. Чтобы лента работала, подключите её следующим образом:

	- **Питание +5V** → к **выводу 5V** на плате
	    
	- **Земля (GND)** → к **GND**
	    
	- **Сигнальный провод (DIN)** → к **GPIO21**

<!--Обратитесь к инструкции по сборке для подробностей.-->
2. Включите поддержку LED-ленты в файле `~/technic_ws/src/technic/technic/launch/technic.launch`:

    ```xml
    <arg name="led" default="true"/>
    ```

3. Настройте параметры подключения ленты *ws281x* в файле `~/technic_ws/src/opi_ws2812_rt/base.launch`. Необходимо ввести верное количество светодиодов в ленте и GPIO-пин, использованный для подключения (если он отличается от *GPIO21*):
...
Technic умеет показывать LED-лентой текущее состояние полетного контроллера и сигнализировать о событиях. Данная функция настраивается в файле `~/technic_ws/src/technic/technic/launch/led.launch` в разделе *events effects table*. Пример настройки:
...
<table>
  ...
  <tr><td><code>startup</code></td><td>Запуск всех систем Technic</td><td>Белый</div></td></tr>
  <tr><td><code>connected</code></td><td>Успешное подключение к полетному контроллеру</td><td>Цвета российского флага</td></tr>
...
> **Заметки** Для корректной работы сигнализации LED-лентой о низком заряде батареи необходимо корректная [калибровка электропитания](EnergySettings.md).
