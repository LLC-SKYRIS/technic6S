...
Для работы с основной камерой необходимо убедиться что она включена в файле `~/technic_ws/src/technic/technic/launch/technic.launch`:
...
Если изображение с камеры отсутствует, попробуйте проверить ее с помощью утилиты [`ffplay`]().
...
Получите картинку с камеры утилитой `ffplay`:

```bash
ffplay /dev/video0
```

> **Подсказка** Путь к устройству может отличаться (`/dev/video0`, `/dev/video1` и т.д.) в зависимости от USB-порта, в который подключена камера.

Если команда завершается с ошибкой, проверьте качество подключения шлейфа камеры к OPi или замените его.
...
Для реализации алгоритмов компьютерного зрения рекомендуется использовать предустановленную на eMMC библиотеку [OpenCV](https://opencv.org).
...
from technic import long_callback
...
> **Заметки** Обработка изображения может занимать значительное время. Это может вызвать [проблему](https://github.com/ros/ros_comm/issues/1901) в библиотеке rospy, которая приведет к обработке устаревших кадров с камеры. Для решения этой проблемы необходимо использовать декоратор `long_callback` из библиотеки `technic`, как в примере выше.
...
> **Подсказка** Для высокоскоростного распознавания и позиционирования лучше использовать [ArUco-маркеры](ArucoMarker.md).
...
> **Подсказка** Смотрите другие примеры по работе с компьютерным зрением в каталоге `~/examples` [образа для OPI](ImageOPI.md).
