...
> **Подсказка** Для распознавания маркеров модуль камеры должен быть корректно подключен и [сконфигурирован](CameraSettings.md).
...
<img src="assets/aruco-map-axis.png" width="300">
...
Для [автономных полетов](AutonomousFlightOffboard.md) можно будет использовать функции `navigate`, `set_position`, `set_velocity`. Для полета в определенные координаты маркерного поля необходимо использовать фрейм `aruco_map`:
...
По аналогии с [навигацией по отдельным маркерам](ArucoMarker.md#навигация-по-маркерам) при настройке карты маркеров дрон сможет лететь в координаты относительно отдельного маркера, используя фрейм `aruco_ID` с соответствующим ID маркера.
...
<img src="assets/IMG_4175.jpg" width="300">


Для навигации по маркерам, расположенным на потолке, необходимо поставить основную камеру так, чтобы она смотрела вверх и [установить соответствующий фрейм камеры](CameraSettings.md).
...
Технология [Optical Flow](OpticalFlow.md) не может нормально работать при таком расположении камеры, поэтому в файле `~/technic_ws/src/technic/technic/launch/technic.launch` ее следует отключить:
