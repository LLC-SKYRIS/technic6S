...
* `frame_id` – [система координат](CoordinateSystems.md), в которой заданы `x`, `y`, `z` и `yaw` (по умолчанию: `map`).

> **Заметки** Для полета без изменения угла по рысканью достаточно установить `yaw` в `NaN` (значение угловой скорости по умолчанию – 0).
...
Полет в точку 3:2 (высота 2) в системе координат [маркерного поля](ArucoMap.md) со скоростью 1 м/с:
...
* `speed` – скорость полета (скорость движения setpoint) *(м/с)*;
* `auto_arm` – перевести коптер в `OFFBOARD` и заармить автоматически (**коптер взлетит**);
* `frame_id` – [система координат](CoordinateSystems.md), в которой заданы `z` и `yaw` (по умолчанию: `map`).

> **Заметки** Для полета без изменения угла по рысканью достаточно установить `yaw` в `NaN`.
...
* `z` – высота полета *(м)*;
* `frame_id` – [система координат](CoordinateSystems.md) для расчета высоты полета.
...
Установить высоту полета в 1 м относительно [маркерного поля](ArucoMap.md):
...
* yaw – угол по рысканью *(радианы)*;
* frame_id – [система координат](CoordinateSystems.md) для расчета угла по рысканью.
...
Установить угол по рысканью в ноль в системе координат [маркерного поля](ArucoMap.md):
...
* `yaw` – угол по рысканью *(радианы)*;
* `auto_arm` – перевести коптер в `OFFBOARD` и заармить автоматически (**коптер взлетит**);
* `frame_id` – [система координат](CoordinateSystems.md), в которой заданы `vx`, `vy`, `vz` и `yaw` (по умолчанию: `map`).

> **Заметки** Параметр `frame_id` определяет только ориентацию результирующего вектора скорости, но не его длину.
...
* `auto_arm` – перевести коптер в `OFFBOARD` и заармить автоматически (**коптер взлетит**);
* `frame_id` – [система координат](CoordinateSystems.md), в которой задан `yaw` (по умолчанию: `map`).
...
> **Заметки** Для автоматического отключения винтов после посадки [параметр PX4](Parameters.md) `COM_DISARM_LAND` должен быть установлен в значение > 0.
...
> **Подсказка** В более новых версиях PX4 коптер выйдет из режима LAND в ручной режим, если сильно перемещать стики.
