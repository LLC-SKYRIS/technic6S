...
Для того, чтобы открыть интерфейс блочного программирования в Tehcnic, [подключитесь к Technic по Wi-Fi](ConnectingToWi-Fi.md) и перейдите на страницу http://10.42.0.1:8000/technic_blocks/ либо нажмите ссылку *Blocks programming* на [основной веб-странице Technic](WebInterface.md).
...
Набор блоков приблизительно аналогичен набору ROS-сервисов [API автономных полетов Technic](AutonomousFlightOffboard.md). В этом разделе приведено описание некоторых из них.
...
* <span style="padding:2px;color:white;background:#01d754">LED</span> – блоки для управления [LED-лентой](Программирование/Периферия/Сенсорная_периферия/LedStrip.md).
* <span style="padding:2px;color:white;background:#5b97cc">GPIO</span> – блоки для работы с [GPIO-пинами](Программирование/Периферия/Сенсорная_периферия/GPIO.md).
...
В блоке может быть выбрана [система координат](CoordinateSystems.md), в которой задана целевая точка:

* *body* – координаты относительно коптера: вперед (*forward*), влево (*left*), вверх (*up*).
* *markers map* – система координат, связанная с [картой ArUco-маркеров](ArucoMap.md).
* *marker* – система координат, связанная с [ArUco-маркером](Aruco.md); появляется поле для ввода ID маркеа.
