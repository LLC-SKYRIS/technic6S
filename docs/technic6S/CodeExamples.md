...
Ожидание окончания прилета в [navigate](AutonomousFlightOffboard.md)-точку:
...
Функция определения расстояния между двумя точками (**важно**: точки должны быть в одной [системе координат](CoordinateSystems.md)):
...
Трансформировать позицию (`PoseStamped`) из одной системы координат ([фрейма](CoordinateSystems.md)) в другую, используя [tf2](http://wiki.ros.org/tf2):
...
Перед выполнением флипа необходимо принять все меры безопасности.
### # {#calibrate-gyro}
